...
class GeminiService {
  ...
  Future<Map<String, dynamic>> generateUiSpecs(String userPrompt, String platform) async {
    // Simulate Network Delay
    await Future.delayed(const Duration(milliseconds: 1000));

    // Return rich mock data based on simple keyword matching
    if (userPrompt.toLowerCase().contains('crypto') || userPrompt.toLowerCase().contains('finance')) {
      return _cryptoMock;
    } else if (userPrompt.toLowerCase().contains('social') || userPrompt.toLowerCase().contains('chat')) {
      return _socialMock;
    } else {
